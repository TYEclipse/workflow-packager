...
---
# Workflow Packager — 工作流 CRUD 执行引擎 v3.3.8
## 角色定位
...
5. **输出基线报告**：生成 Markdown 表格，列：`名称 | 类型(skill/cron/subagent) | 最后活跃 | 引用计数 | 状态`
   ...
   - 引用计数：search_files(pattern='<name>', target='content') 跨 skills/memories/cron 统计；`.usage.json`、hub 索引缓存、历史报告回显均属**非功能引用**，统计时须排除
...
1. **确认僵尸清单**：从 Phase 0 基线报告的 🧟 Zombie 列表出发，对每个候选执行最后确认：
   ...
   - **curator 归属复核**：若环境存在技能归档 curator（如 Hermes `curator`），先查名单命令（如 `hermes curator list-unmanaged`）确认归属——**已被 curator 管辖（有 provenance 标记）的技能交由其归档通道处理（按阈值自动归档、可 restore），本流水线不抢先删除**；本流水线仅删除 curator 不可达（无 provenance / 在免疫名单）的已确认僵尸
   ...
   - **可重装核验**：删除前确认该技能存在上游可恢复源（官方 optional-skills 在位、hub 渠道可重装等）——无源可重装者一律不删。**核验三查（实测路径，本机无 `~/.hermes/optional-skills` 目录，须实查而非假设）**：① `hermes curator usage --json` 的 `provenance` 字段（bundled/hub/agent）② `~/.hermes/skills/.hub/index-cache/hermes-index.json` 命中 ③ `~/.hermes/skills/.skills_store_lock.json` 的 `zip_url/source/version`。②③ 是**子串匹配**，须人工复核命中条目（短名易被别的条目描述误命中，如 `github-issues` 命中别的 skill 正文）；三查皆无 ⇒ 无源可重装 ⇒ 规则封顶保留（官方套件叶子常属此类）
   - **mtime 判龄纪律**：判龄用技能目录自身 mtime（`stat -c %y`），不要用移动/复制后产生的容器时间戳
...
1. **确认重复对**：基于三轮检测结果（关键词聚类 + 触发词碰撞 + 功能意图），筛选置信度 ≥ 高 的对
   ...
   - **curator 管辖提示**：源技能若已被 curator 管辖（provenance 非空），curator 的 consolidate 通道（可能处于 off）与本流水线都可归并——报告里写明现状（如 `consolidate: off`），让用户选择用哪条通道，不默认抢跑
2. **干运行合并计划**：
   ```
...
- **当日修改技能体检**：每轮运行对**当日修改过的每个技能**执行三项检查：
  ...
  2. **合规检查**：frontmatter 无 author 真名、无变更日志章节、版本号四处一致（`VERSION`/frontmatter/H1/README 徽章，`python3 tools/bump_version.py --show` 自查）
